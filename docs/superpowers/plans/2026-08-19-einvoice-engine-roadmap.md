...
| 5 | Plan 5 — Onboarding dashboard (UI/UX) | Own spec: `2026-08-20-onboarding-dashboard-design.md` (brainstormed + designed 2026-08-20) | Merchant-facing dashboard served by the engine app (Inertia v2 + React + TS, Approach A: session-authenticated web controllers over the shared domain actions). Magic-link auth (Mailtrap), invite-only users with owner/member/vendor roles (vendors issuer-scoped). Areas: issuer onboarding wizard (profile → TIN verify → LHDN mode → certificate → sandbox test → go live), API keys, webhooks, document browser, marketplace vendor onboarding with progress board. |

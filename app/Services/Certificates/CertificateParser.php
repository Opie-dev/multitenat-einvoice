...
class CertificateParser
{
    ...
    private function build(OpenSSLCertificate $cert, OpenSSLAsymmetricKey $key): CertificateInfo
    {
        ...
        // config at module init time, before phpunit's <env OPENSSL_CONF> takes
        // effect, so openssl_pkey_export() fails with a "configuration file
        // routines" error even though OPENSSL_CONF is set by request time.
        // Passing the config path explicitly sidesteps that timing issue; on
        // Linux/production, where it's unset, this is a no-op.
        $opensslConf = config('services.openssl_conf');
        $exportOptions = is_string($opensslConf) && $opensslConf !== '' ? ['config' => $opensslConf] : [];
        openssl_pkey_export($key, $keyPem, null, $exportOptions); // unencrypted PEM; encrypted at rest by the model cast
